...
from DataFormat import Muon, Electron, Jet, Particle, Tau, FatJet
...
def getTaus(evt):
    taus = []
    taus_zip = zip(evt.TauPtColl,
                   evt.TauEtaColl,
                   evt.TauPhiColl,
                   evt.TauMassColl,
                   evt.TauChargeColl,
                   evt.TauGenPartFlavColl,
                   evt.TauDecayModeColl)
    for pt, eta, phi, mass, charge, genpartflav, decaymode in taus_zip:
        thisTau = Tau(pt,eta,phi,mass)
        thisTau.SetCharge(charge)
        thisTau.SetGenPartFlav(genpartflav)
        thisTau.SetDecayMode(decaymode)
        taus.append(thisTau)
...
def getFatJets(evt):
    fatjets = []
    fatjets_zip = zip(evt.FatJetPtColl,
                      evt.FatJetEtaColl,
                      evt.FatJetPhiColl,
                      evt.FatJetMassColl,
                      evt.FatJetChargeColl,
                      evt.FatJetLsf3Coll,
                      evt.FatJetTau3Coll)
    for pt, eta, phi, mass, charge, lsf3, tau3 in fatjets_zip:
        thisFatJet = FatJet(pt, eta, phi, mass)
        thisFatJet.SetCharge(charge)
        thisFatJet.SetLSF3(lsf3)
        thisFatJet.SetTau3(tau3)
        fatjets.append(thisFatJet)
